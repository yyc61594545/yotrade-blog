...
先跑一次 `python3 scripts/pick-next-topic.py`，只看输出，不算本次选题：

- 退出码非 0（池子已空），或输出里 `remaining < 30`：**先补题，再进入第 1 步**
- 否则直接进入第 1 步

补题规则（与 `AGENTS.md`「Topic pool」一致，2026-09-23 起 Claude 也必须执行，之前只有 Codex 顶班时才会补）：

- 至少新增 **60 条** `cn-` 选题到 `scripts/topic-pool.md`，放进一个新的 `###` 小节，小节标题写明日期与来源；**不要改动已有条目**
- slug 唯一（不能与 `src/content/blog/` 下已有文件或池内已有 slug 重复），分类只用已有分类
- 面向国内普通用户在获取 AI 服务上的卡点：注册、手机验证、付款、订阅、国内可用性、价格对比、封号与找回。不写英文开发者向或深度技术选题
- **选题来源优先级**：
...
**手机验证簇（【优先】小节）**：相关阅读必须包含 `/blog/cn-chatgpt-register-without-foreign-phone/`，并至少再内链本簇或注册环节长尾另 1 篇。
...
**CTA 链接**：默认 `https://yotradeapi.com`（**不**带 `/register` 等路径）。API 中转产品已于 2026-08-25 下线，导流段**不得**再写"API 中转 / 统一 API 接入 / 拿 Key 调用"，只能写订阅代充或下面的验证码代收。
...
结束语里写明选题池剩余数量。如果本次在第 0.5 步补过题，写明补了多少条、各来自哪类来源。
